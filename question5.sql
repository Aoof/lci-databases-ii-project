...
BEGIN
  ...
  FROM CITY
  WHERE CITY_NAME = UPPER(P_DEPARTURE_CITY);
  ...
  DBMS_OUTPUT.PUT_LINE(
          ...
          RPAD('DEPARTURE TIME', 17) ||
          RPAD('ARRIVAL TIME', 15) ||
          RPAD('CITY ARRIVAL', 12));
  DBMS_OUTPUT.PUT_LINE(RPAD('-', 90, '-'));

  FOR I IN V_FL_TAB.FIRST .. V_FL_TAB.LAST LOOP
  DBMS_OUTPUT.PUT_LINE(
        RPAD(V_FL_TAB(I).V_FLIGHT_ID, 12) ||
        RPAD(V_FL_TAB(I).V_PILOT_NAM, 14) ||
        RPAD(V_FL_TAB(I).V_PLANE_DESC, 19 ) ||
        RPAD(V_FL_TAB(I).V_DEPARTURE_TIME, 17) ||
        RPAD(V_FL_TAB(I).V_ARRIVAL_TIME, 15) ||
